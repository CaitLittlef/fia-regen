...
###################### MAKE SURE YOU'RE WORKING WITH LATEST DATA!!!
...
data.pipo$regen_pipo <- as.numeric(as.character(data.pipo$regen_pipo))
boo <- data.pipo[data.pipo$YEAR.DIFF > 14,]# & data.pipo$BALive_pipo > 0,]
...
breaks <- 5
folds <- cut(seq(1,nrow(boo)),breaks=breaks,labels=FALSE)
AUC <- NULL
for(i in 1:breaks){
